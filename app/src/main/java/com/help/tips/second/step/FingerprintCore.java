...
import com.help.tips.second.step.CryptoObjectCreator;
...
public class FingerprintCore {
    ...
    public FingerprintCore(Context context) {
        ...
        LogUtils.e("TAG", "fingerprint isSupport: " + isSupport);
        initCryptoObject();
    }
    ...
    private void initCryptoObject() {
        try {
            ...
            LogUtils.e("TAG", "create cryptoObject failed!");
        }
    }
    ...
    private void notifyStartAuthenticateResult(boolean isSuccess, String exceptionMsg) {
        if (isSuccess) {
            LogUtils.e("TAG", "start authenticate...");
            if (mFpResultListener.get() != null) {
                mFpResultListener.get().onStartAuthenticateResult(true);
            }
        } else {
            LogUtils.e("TAG", "startListening, Exception" + exceptionMsg);
            if (mFpResultListener.get() != null) {
    ...
    private void notifyAuthenticationSucceeded() {
        LogUtils.e("TAG", "onAuthenticationSucceeded");
        mFailedTimes = 0;
    ...
    private void notifyAuthenticationError(int errMsgId, CharSequence errString) {
        LogUtils.e("TAG", "onAuthenticationError, errId:" + errMsgId + ", err:" + errString + ", retry after 30 seconds");
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void notifyAuthenticationFailed(int msgId, String errString) {
        LogUtils.e("TAG", "onAuthenticationFailed, msdId: " +  msgId + " errString: " + errString);
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    public void cancelAuthenticate() {
        if (mCancellationSignal != null && mState != CANCEL) {
            LogUtils.e("TAG", "cancelAuthenticate...");
            mState = CANCEL;
    ...
    private void onFailedRetry(int msgId, String helpString) {
        mFailedTimes++;
        LogUtils.e("TAG", "on failed retry time " + mFailedTimes);
        if (mFailedTimes > 5) { // 每个验证流程最多重试5次，这个根据使用场景而定，验证成功时清0
            LogUtils.e("TAG", "on failed retry time more than 5 times");
            return;
        }
        LogUtils.e("TAG", "onFailedRetry: msgId " + msgId + " helpString: " + helpString);
        cancelAuthenticate();
    ...
    public static FingerprintManager getFingerprintManager(Context context) {
        ...
        try {
            ...
            LogUtils.e("TAG", "have not class FingerprintManager");
        }
